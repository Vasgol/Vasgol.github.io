...
function userInput() {
    ...
    if (lastUtterance) {
        ajaxdata.answer = ajaxdata.utterance;
        ajaxdata.utterance = lastUtterance;
        if (prevAnswers) {
            ajaxdata.prevAnswers = prevAnswers
        } else {
            lastUtterance = null;
        }
    }
    ...
    $.ajax({
        ...
        if (result.question) {
            sayUtterance("system", result.question);
            lastUtterance = result.utterance;
            if (result.prevAnswers) {
